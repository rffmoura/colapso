...
function clone(s: GameState): GameState {
  // O estado do motor é deliberadamente serializável. Evitar `structuredClone`
  // mantém o pacote independente de DOM e compatível com Hermes/React Native.
...
function drawInto(
  ...
  for (let i = 0; i < n; i++) {
    if (side.deck.length === 0 && side.discard.length > 0) {
      side.deck = shuffle(side.discard, random)
      side.discard = []
...
export function newGame(
  setup: MatchSetup = defaultMatchSetup(),
  random: () => number = Math.random,
): GameState {
  ...
  const s: GameState = {
    ...
    winner: null,
    nextUid: 1,
    setup: JSON.parse(JSON.stringify(setup)) as MatchSetup,
  }
  const ev: GameEvent[] = []
  drawInto(s, 'player', 4, ev, random)
  drawInto(s, 'ai', 5 + (setup.directives.includes('arquivo-prioritario') ? 1 : 0), ev, random)
  return s
}

export function startTurn(prev: GameState, random: () => number = Math.random): StepResult {
  const s = clone(prev)
  const ev: GameEvent[] = []
  s.turn += 1
  ...
  drawInto(s, s.active, need, ev, random)
...
export function playCreature(
  ...
  if (def.onPlay === 'colapsarInimigo') {
    ...
    if (targets.length > 0) {
      const pick = targets[Math.floor(random() * targets.length)]
...
export function finishProtocol(
  ...
  const ev: GameEvent[] = []
  const defender = other(owner)
  if (triggerSecret(s, defender, 'copia-carbono', ev)) drawInto(s, defender, 2, ev, random)
  return { state: s, events: ev }
}
